...
and will return it according to defined format.
Read will ignore the config file and requires adapter configuration using command line.`,
...
func init() {
	...
	readCmd.PersistentFlags().StringP(
		"type", "t",
		"holding",
		"Register type to read: holding|input|coil",
	)
	readCmd.PersistentFlags().StringP(
		"encoding", "e",
		"int",
...
func decodeCoils(b []byte) string {
	s := ""
	for i := 0; i < len(b); i += 2 {
		if len(s) > 0 {
			s += " "
